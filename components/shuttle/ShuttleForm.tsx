...
export default function ShuttleForm() {
  ...
  const availableTimesSet = useMemo(() => new Set(availableTimes), [availableTimes])

  const availableHours = useMemo(() => {
    const hourSet = new Set<string>()
    for (const slot of availableTimes) {
      hourSet.add(slot.split(':')[0])
    }
    return hourSet
  }, [availableTimes])
  ...
  useEffect(() => {
    ...
    async function loadAvailability() {
      ...
      try {
        ...
        if (form.pickupTime && !slots.includes(form.pickupTime)) {
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 bg-white focus-within:ring-2 focus-within:ring-ocean-400 ${
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const mins = form.pickupTime ? form.pickupTime.split(':')[1] : ''
                        const candidate = selectedHour && mins ? `${selectedHour}:${mins}` : ''
                        let newTime = ''

                        if (candidate && availableTimesSet.has(candidate)) {
                          newTime = candidate
                        } else if (selectedHour) {
                          const fallback = availableTimes.find((slot) =>
                            slot.startsWith(`${selectedHour}:`),
                          )
                          newTime = fallback ?? ''
                        }
                      ...
                      disabled={!form.travelDate || availabilityLoading}
                    >
                      ...
                      {HOURS.map(({ value, label }) => (
                        <option
                          key={value}
                          value={value}
                          disabled={form.travelDate ? !availableHours.has(value) : false}
                        >
                          {label}
                        </option>
                      ))}
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (!hrs) return
                        const candidate = `${hrs}:${e.target.value}`
                        if (!availableTimesSet.has(candidate)) return
                        set('pickupTime', candidate)
                      }}
                      className="w-16 text-sm text-ocean-950 bg-transparent focus:outline-none cursor-pointer"
                      disabled={!form.travelDate || !form.pickupTime || availabilityLoading}
                    >
                      {MINUTES.map(({ value, label }) => (
                        <option
                          ...
                          disabled={
                            !!form.pickupTime &&
                            !availableTimesSet.has(`${form.pickupTime.split(':')[0]}:${value}`)
                          }
                        >
                          {label}
                        </option>
                  ...
                  {form.travelDate && !availabilityLoading && availableTimes.length === 0 && (
                    <p className="text-amber-600 text-xs mt-1.5">
                      {tf('availabilityEmpty')}
                    </p>
                  )}
                  {availabilityLoading && (
                    <p className="text-gray-400 text-xs mt-1.5">{tf('availabilityLoading')}</p>
                  )}
