...
import { getShuttleWindow } from '@/lib/shuttleAvailability'
  ...
  return (
    <div className="min-h-screen bg-ocean-950 flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white rounded-2xl p-8 shadow-xl">
        ...
        <p className="text-gray-500 text-sm mb-6">Ridewave Shuttle</p>
...
function BookingCard({ b, isToday }: { b: Booking; isToday: boolean }) {
  ...
  const blockedRange = isDestination(b.destination)
    ? getShuttleWindow({
        travelDate: b.travel_date,
        pickupTime: b.pickup_time,
        destination: b.destination,
        isRoundTrip: b.is_round_trip === true,
      })
    : null
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 text-sm mb-4">
        ...
        {blockedRange && (
          <div className="flex items-center gap-1.5 text-amber-700 col-span-2">
            <Clock className="w-4 h-4 text-amber-500 shrink-0" />
            Bloque ocupado: {formatTime(blockedRange.startAt)} - {formatTime(blockedRange.endAt)}
          </div>
        )}
        <div className="flex items-center gap-1.5 text-gray-600">
